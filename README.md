...
2. In the SQL Editor, run each file in `supabase/migrations/` **in order**
   (0001 → 0032, skipping 0005 — removed along with the Captain/VC feature).
...
3. Start Draft. The nominator rotates in join order; nominating a horse
   ...
   clock at all. A 30s nomination clock auto-nominates a random horse
...
- **Weekly transfers are waiver-based, not first-come-first-served.**
  ...
  priority order; nothing happens until the window closes (Saturday 8:30am
