...
--   - bid clock is 10s and RESETS on every accepted bid (soft close)
...
begin
  ...
  if v_state.status = 'nominating' and v_state.nomination_deadline is not null and v_state.nomination_deadline <= now() then
    ...
    update public.league_draft_state set
      ...
      bid_count = 1,
      status = 'bidding',
      bid_deadline = now() + interval '10 seconds',
      updated_at = now()
    where league_id = p_league_id
    returning * into v_state;
...
-- any higher amount up to their max bid. Resets the 10s bid clock on success.
-- ----------------------------------------------------------------------------
...
begin
  ...
  select max_bid into v_max_bid from public.league_budget(p_league_id, auth.uid());
